...
public class AdRaceCodeDaoImpl extends GenericDaoSupport<Long, AdRaceCode> implements AdRaceCodeDao {
    ...
    public AdRaceCodeDaoImpl() {
        super(AdRaceCodeImpl.class);
    }

    @Override
    public AdRaceCode findByCode(String code) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select s from AdRaceCode s where s.code = :code and  " +
    ...
    public List<AdRaceCode> find(String filter, Integer offset, Integer limit) {
        ...
        query.setCacheable(true);
        return (List<AdRaceCode>) query.list();
    ...
    public Integer count(String filter) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(s) from AdRaceCode s where " +
                "(upper(s.code) like upper(:filter) " +
                "or upper(s.description) like upper(:filter)) " +
                "and s.metadata.state = :state ");
        query.setString("filter", WILDCARD + filter + WILDCARD);
    ...
    public boolean isExists(String code) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(*) from AdRaceCode s where " +
                "s.code = :code " +
                "and s.metadata.state = :state ");
        query.setString("code", code);
        query.setInteger("state", AdMetaState.ACTIVE.ordinal());
