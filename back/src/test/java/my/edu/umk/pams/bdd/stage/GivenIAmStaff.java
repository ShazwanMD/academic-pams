...
@JGivenStage
public class GivenIAmStaff extends Stage<GivenIAmStaff> {
	
	private static final Logger LOG = LoggerFactory.getLogger(GivenIAmStaff.class);
	  ...
	  @Autowired
	    ...
	    public GivenIAmStaff I_am_a_staff_in_$_academic_session(String academicSessionCode) {
	        loginAsStaff();
	        academicSession = plannerService.findAcademicSessionByCode(academicSessionCode);
	        return self();
	    }

	    public GivenIAmStaff I_am_a_staff_in_current_academic_session() {
	        loginAsStaff();
	        academicSession = plannerService.findCurrentAcademicSession();
	        return self();
	    }

	    public GivenIAmStaff I_pick_program_$(String code) {
	        program = plannerService.findProgramByCode(code);
	        return self();
	    }

	    public GivenIAmStaff I_pick_offering_$(String canonicalCode) {
	        offering = termService.findOfferingByCanonicalCode(canonicalCode);
	        return self();
	    }

	    public GivenIAmStaff I_pick_section_$(String canonicalCode) {
