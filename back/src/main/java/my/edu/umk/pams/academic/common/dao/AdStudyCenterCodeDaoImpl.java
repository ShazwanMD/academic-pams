...
@Repository("adStudyCenterCodeDao")
public class AdStudyCenterCodeDaoImpl extends GenericDaoSupport<Long, AdStudyCenterCode> implements AdStudyCenterCodeDao {

    private static final Logger LOG = LoggerFactory.getLogger(AdStudyCenterCodeDaoImpl.class);

    public AdStudyCenterCodeDaoImpl() {
        super(AdStudyCenterCodeImpl.class);
    }

    @Override
    public AdStudyCenterCode findByCode(String code) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select s from AdStudyCenterCode s where " +
                "s.code = :code " +
    ...
    public List<AdStudyCenterCode> find(String filter, Integer offset, Integer limit) {
        ...
        query.setCacheable(true);
        return (List<AdStudyCenterCode>) query.list();
    }

    @Override
    public Integer count(String filter) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(s) from AdStudyCenterCode s where " +
                "(upper(s.code) like upper(:filter) " +
                "or upper(s.description) like upper(:filter)) " +
                "and s.metadata.state = :state ");
        query.setString("filter", WILDCARD + filter + WILDCARD);
    ...
    public boolean isExists(String code) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(*) from AdStudyCenterCode s where " +
                "s.code = :code " +
                "and s.metadata.state = :state ");
        query.setString("code", code);
        query.setInteger("state", AdMetaState.ACTIVE.ordinal());
