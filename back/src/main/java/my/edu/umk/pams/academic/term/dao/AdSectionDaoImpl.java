...
public class AdSectionDaoImpl extends GenericDaoSupport<Long, AdSection> implements AdSectionDao {
	...
	public AdSection findByCanonicalCode(String canonicalCode) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s from AdSection s where " + "s.canonicalCode = :canonicalCode "
	...
	@Override
	public AdSection findByCodeAndSessionAndOffering(String code, AdAcademicSession academicSession,
			AdOffering offering) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session
				.createQuery("select s from AdSection s where " + "s.code = :code " + "and s.offering = :offering "
	...
	@Override
	public List<AdSection> find(String filter, AdAcademicSession academicSession, AdAdmission admission, Integer offset,
			Integer limit) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s from AdSection s where " + "(upper(s.code) like upper(:filter) "
				+ "or upper(s.offering.code) like upper(:filter) " + "or upper(s.offering.title) like upper(:filter)) "
				+ "and s.session = :academicSession " + "and s.metadata.state = :state " + "order by s.code asc");
		query.setString("filter", WILDCARD + filter + WILDCARD);
		query.setEntity("academicSession", academicSession);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setFirstResult(offset);
	...
	public List<AdSectionPolicy> findPolicies(AdSection section) {
		...
		Query query = session.createQuery(
				"select s from AdSectionPolicy s where " + "s.section = :section " + "and s.metadata.state = :state ");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public List<AdEnrollment> findEnrollments(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s from AdEnrollment s where " + "s.section = :section "
				+ "and s.metadata.state = :state " + "order by s.student.identityNo asc");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("section", section);
		query.setCacheable(true);
	...
	public List<AdAppointment> findAppointments(AdOffering offering) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s from AdAppointment s where " + "s.section.offering = :offering "
				+ "and s.metadata.state = :state");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("offering", offering);
		query.setCacheable(true);
	...
	public List<AdAppointment> findAppointments(AdAcademicSession academicSession, AdOffering offering) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s from AdAppointment s where " + "s.section.offering = :offering "
				+ "and s.section.offering.session = :academicSession " + "and s.metadata.state = :state");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("offering", offering);
		query.setEntity("academicSession", academicSession);
		query.setCacheable(true);
	...
	public List<AdAppointment> findAppointments(AdCourse course) {
		...
		Query query = session.createQuery("select s from AdAppointment s where "
				+ "s.section.offering.course = :course " + "and s.metadata.state = :state");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public List<AdAppointment> findAppointments(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery(
				"select s from AdAppointment s where " + "s.section = :section " + "and s.metadata.state = :state");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("section", section);
		query.setCacheable(true);
		return (List<AdAppointment>) query.list();
	...
	public List<AdStudent> findStudents(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s.student from AdEnrollment s where " + "s.section = :section "
				+ "and s.metadata.state = :state " + "order by s.student.name");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("section", section);
		query.setCacheable(true);
	...
	public List<AdStaff> findStaffs(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select s.staff from AdAppointment s where " + "s.section = :section "
				+ "and s.metadata.state = :state ");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		query.setEntity("section", section);
		query.setCacheable(true);
	...
	public Integer count() {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdSection s where " + "s.metadata.state = :state ");
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public Integer count(String filter, AdAcademicSession academicSession) {
		...
		Query query = session.createQuery("select count(s) from AdSection s where "
				+ "(upper(s.code) like upper(:filter) " + "or upper(s.offering.code) like upper(:filter) "
				+ "or upper(s.offering.course.title) like upper(:filter)) " + "and s.session = :academicSession "
				+ "and s.metadata.state = :state ");
		query.setString("filter", WILDCARD + filter + WILDCARD);
		query.setEntity("academicSession", academicSession);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public Integer count(AdOffering offering) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdSection s where " + "s.offering = :offering  "
				+ "and s.metadata.state = :state ");
		query.setEntity("offering", offering);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer count(AdCourse course) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdSection s where " + "s.offering.course = :course  "
				+ "and s.metadata.state = :state ");
		query.setEntity("course", course);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer count(AdAcademicSession academicSession) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdSection s where " + "s.session = :academicSession  "
				+ "and s.metadata.state = :state ");
		query.setEntity("academicSession", academicSession);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer count(AdAcademicSession academicSession, AdCourse course) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdSection s where " + "s.session = :academicSession  "
				+ "and s.offering.course = :course  " + "and s.metadata.state = :state ");
		query.setEntity("academicSession", academicSession);
		query.setEntity("course", course);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer countStudent(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdEnrollment s where " + "s.section = :section  "
				+ "and s.metadata.state = :state ");
		query.setEntity("section", section);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer countAvailableStudent(AdOffering offering) {
		...
		Query query = session.createQuery("select count(s) from AdEnrollment e right join e.student s "
				...
				+ "(c is null or (c is not null and c <> :offering)) " + "and s.metadata.state = :state ");
		query.setEntity("offering", offering);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	}
	...
	public Integer countAvailableStudent(AdCourse course) {
		...
		Query query = session.createQuery("select count(s) from AdEnrollment e right join e.student s "
				...
				+ "(c is null or (c is not null and c <> :course)) " + "and s.metadata.state = :state ");
		query.setEntity("course", course);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	}
	...
	public Integer countAvailableStudent(String filter, AdOffering offering) {
		...
		Query query = session.createQuery("select count(s) from AdStudent s where "
				+ "s not in (select e.student from AdEnrollment e where " + "e.section.offering = :offering) "
				+ "and (upper(s.identityNo) like upper(:filter) " + "or upper(s.name) like upper(:filter) "
				+ "or upper(s.credentialNo) like upper(:filter)) " + "and s.metadata.state = :state ");
		query.setEntity("offering", offering);
		query.setString("filter", WILDCARD + filter + WILDCARD);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public Integer countAvailableStudent(String filter, AdCourse course) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdStudent s where "
				+ "s not in (select e.student from AdEnrollment e where " + "e.section.offering.course = :course) "
				+ "and (upper(s.identityNo) like upper(:filter) " + "or upper(s.name) like upper(:filter) "
				+ "or upper(s.credentialNo) like upper(:filter)) " + "and s.metadata.state = :state ");
		query.setEntity("course", course);
		query.setString("filter", WILDCARD + filter + WILDCARD);
	...
	public Integer countAvailableStaff(AdAcademicSession academicSession, AdOffering offering) {
		...
		Query query = session.createQuery("select count(s) from AdAppointment e right join e.staff s "
				+ "left join e.section t left join t.offering c where "
				+ "(c is null or (c is not null and c <> :offering)) " + "and t.session = :academicSession "
				+ "and s.metadata.state = :state ");
		query.setEntity("offering", offering);
		query.setEntity("academicSession", academicSession);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public Integer countAppointment(AdSection section) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(s) from AdAppointment s where " + "s.section = :section  "
				+ "and s.metadata.state = :state ");
		query.setEntity("section", section);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return ((Long) query.uniqueResult()).intValue();
	...
	public boolean hasSection(AdAcademicSession academicSession) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(*) from AdSection s where " + "s.session = :academicSession "
				+ "and s.metadata.state = :state ");
		query.setEntity("academicSession", academicSession);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
	...
	public boolean isExists(String canonicalCode) {
		...
		Query query = session.createQuery("select count(*) from AdSection s where "
				+ "s.canonicalCode = :canonicalCode " + "and s.metadata.state = :state ");
		query.setString("canonicalCode", canonicalCode);
		query.setInteger("state", AdMetaState.ACTIVE.ordinal());
		return 0 < ((Long) query.uniqueResult()).intValue();
	}
	...
	public boolean isAppointmentExists(AdSection section, AdStaff staff) {
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("select count(*) from AdAppointment s where " + "s.section = :section "
				+ "and s.staff = :staff " + "and s.metadata.state = :state ");
		query.setEntity("section", section);
