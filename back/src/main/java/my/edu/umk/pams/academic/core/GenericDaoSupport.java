...
//        String queryString = RECURSIVE_ACL_QUERY + String.format(" where AC.CLASS = '%s' and SID.SID in (%s) ", interfaceClass.getCanonicalName(), sidsToStringArray(sids));
    ...
    public void save(I entity, AdUser user) {
    ...
    public void saveOrUpdate(I entity, AdUser user) {
        Validate.notNull(user, "User cannot be null");
        Validate.notNull(entity, "Object cannot be null");
    ...
    public void update(I entity, AdUser user) {
        Validate.notNull(user, "User cannot be null");
        Validate.notNull(entity, "Object cannot be null");
    ...
    public void deactivate(I entity, AdUser user) {
        Validate.notNull(user, "User cannot be null");
        Validate.notNull(entity, "Object cannot be null");
     ...
     * @param user
     */
    public void remove(I entity, AdUser user) {
        Validate.notNull(user, "User cannot be null");
        Validate.notNull(entity, "Object cannot be null");
    ...
    @Override
    public void delete(I entity, AdUser user) {
        Validate.notNull(user, "User cannot be null");
        Validate.notNull(entity, "Object cannot be null");
    ...
    private void prepareMetadata(I i, AdUser user, boolean active) {
        if (i instanceof AdMetaObject) {
            AdMetadata metadata = null;
            if (((AdMetaObject) i).getMetadata() != null)
                metadata = ((AdMetaObject) i).getMetadata();
            else
                metadata = new AdMetadata();
            metadata.setCreatedDate(new Timestamp(System.currentTimeMillis()));
            metadata.setCreatorId(user.getId());
            metadata.setState(active ? AdMetaState.ACTIVE : AdMetaState.INACTIVE);
            ((AdMetaObject) i).setMetadata(metadata);
        }
    }

    private void prepareFlowData(I i, AdUser user) {
        if (i instanceof AdFlowObject) {
            AdFlowdata flowData = null;
            if (((AdFlowObject) i).getFlowdata() != null)
                flowData = ((AdFlowObject) i).getFlowdata();
            else
                flowData = new AdFlowdata();
            flowData.setDraftedDate(new Timestamp(System.currentTimeMillis()));
            flowData.setDrafterId(user.getId());
            flowData.setState(AdFlowState.DRAFTED);
            ((AdFlowObject) i).setFlowdata(flowData);
        }
    }
