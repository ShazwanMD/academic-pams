...
export class ProfileEffects {
    ...
    @Effect() addMinAmount$ = this.actions$
        .ofType(ProfileActions.ADD_MIN_AMOUNT)
        .map(action => action.payload)
        .switchMap(payload => this.profileService.addMinAmount(payload))
        .map(message => this.profileActions.addMinAmountSuccess(message))
        .withLatestFrom(this.store$.select(...this.STUDENT))
        .map(state => state[1])
        .map((student: Student) => this.profileActions.findStudentByIdentityNo(student.identityNo));
