...
@Component( {
    selector: 'pams-student-admission-application-creator',
    templateUrl: './student-admission-application-creator.dialog.html',
} )
    ...
    save( admissionApplication: AdmissionApplication, isValid: boolean ): void {
        ...
        this.admissionApplication$.subscribe( val => {
            if ( val['status'] == 'Duplicate' ) {
                ...
                let snackBarRef = this.snackBar.open( 'Process failed: ' + admissionApplication.student.identityNo + ' Application cannot be submit', 'OK', { duration: 2000 } );
