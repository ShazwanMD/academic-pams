...
export class CurriculumActionComponent {
    ...
    delate(): void {
        let snackBarRef = this.snackBar.open( 'Delate Curriculum', 'OK' , { duration: 2000 }  );
        // snackBarRef.afterDismissed().subscribe(() => this.store.dispatch(this.actionsq.delateCurriculum(this.curriculum)));
    }

    //download report
    downloadReport( reportId, parameterReport: Curriculum ): void {
        let repParam = reportId + '&curriculum_code=' + parameterReport.code;
