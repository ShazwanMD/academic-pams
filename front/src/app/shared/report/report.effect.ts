...
export class ReportEffects {
  ...
  @Effect() downloadReport = this.actions$
    ...
    .switchMap((payload) => this.reportService.downloadReport(payload))
    .map((file) => {
  ...
  @Effect() downloadReportFinance = this.actions$
    .ofType(ReportActions.DOWNLOAD_FINANCE_REPORT)
    .map((action) => action.payload)
    .switchMap((payload) => this.reportService.downloadReportFinance(payload))
    .map((file) => {
      let url = URL.createObjectURL(file);
      window.open(url);
    }).ignoreElements();
