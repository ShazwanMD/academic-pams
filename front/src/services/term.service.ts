...
export class TermService {
  ...
  addAssessment(offering: Offering, assessment: Assessment): Observable<String> {
    console.log("addAssessment:" + assessment.canonicalCode);
    let headers = new Headers({
      'Content-Type': 'application/json',
      //'Authorization': 'Bearer ' + this.authService.token
    });
  ...
  saveOffering(offering: Offering): Observable<String> {
    ...
    let options = new RequestOptions({headers: headers});
    return this.http.post(environment.endpoint + '/api/term/offerings/' + offering.code, JSON.stringify(offering), options)
      .flatMap((res: Response) => Observable.of(res.text()));
  }
